...
flowchart TD
    ...
    Resend["Resend<br/>confirmation and broadcasts"]
    Visitor["Visitor"]
...
The site uses Cloudflare Workers with Static Assets rather than Cloudflare Pages. Static HTML, CSS, JavaScript, images, feeds, and sitemaps are generated by Astro. Hashed assets, images, media, social images, favicons, feeds, robots, and sitemaps bypass the Worker. HTML requests pass through the Worker only to preserve canonical host redirects, then fall through to Assets. Exact legacy path redirects are handled by Static Assets through `_redirects`.
...
| `/api/newsletter/subscriptions` | `POST` | Turnstile-protected double-opt-in requests |
| `/api/newsletter/confirm` | `POST` | Consume a confirmation token and schedule provider synchronization |
